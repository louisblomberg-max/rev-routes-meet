 ...
 * Includes live location sharing, overview mode, and arrival detection.
...
export interface LiveSharingState {
  isSharing: boolean;
  recipients: LiveShareRecipient[];
  sessionId: string | null;
  startedAt: string | null;
}

export interface LiveShareRecipient {
  id: string;
  type: 'friends' | 'specific' | 'club';
  label: string;
}
...
interface NavigationContextType extends NavigationState {
  ...
  setLiveSharingRecipients: (recipients: LiveShareRecipient[]) => void;
...
export const NavigationProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [liveSharing, setLiveSharing] = useState<LiveSharingState>({
    isSharing: false,
    recipients: [],
    sessionId: null,
    startedAt: null,
  ...
  const toggleLiveSharing = useCallback(() => {
    setLiveSharing(prev => ({
      ...
      sessionId: !prev.isSharing ? crypto.randomUUID() : null,
      startedAt: !prev.isSharing ? new Date().toISOString() : null,
    }));
  }, []);

  const setLiveSharingRecipients = useCallback((recipients: LiveShareRecipient[]) => {
    setLiveSharing(prev => ({ ...prev, recipients }));
  }, []);
...
// Local haversine for arrival detection
