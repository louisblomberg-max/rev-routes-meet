...
const FeatureSlide = ({ title, description, highlights, gradient, slideIndex, totalSlides }: Props) => {
  ...
  return (
    <div className="flex-1 flex flex-col relative">
      ...
      {/* Content */}
      <div className="flex-1 flex flex-col items-center justify-center px-8 text-center relative z-10 bg-[#f3f3e8] text-[#fbf4f4]">
        <div className="w-20 h-20 rounded-2xl border flex items-center justify-center mb-8 animate-scale-up bg-white border-secondary" key={slideIndex}>
        ...
        <p className="text-base leading-relaxed max-w-[300px] mb-8 animate-fade-up text-black" key={`d-${slideIndex}`}>
          {description}
        </p>

        <div className="space-y-3 w-full max-w-[280px] animate-fade-up text-black" key={`h-${slideIndex}`}>
          {highlights.map((h, i) =>
          <div key={i} className="flex items-center gap-3 text-left">
              <div className="w-1.5 h-1.5 rounded-full bg-primary shrink-0" />
              <span className="text-sm text-accent">{h}</span>
            </div>
          )}
        </div>
      </div>
