...
const BottomNavigation = ({ activeTab, onTabChange }: BottomNavigationProps) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 safe-bottom px-4 pb-3 pt-1" style={{ backgroundColor: '#f3f3e8' }}>
      <div className="max-w-md mx-auto bg-white rounded-2xl border border-black/10 shadow-sm flex items-center justify-around py-1.5 px-2">
        {tabs.map((tab) => {
          const Icon = tab.icon;
