...
export default function CommunitySOSView() {
  ...
  return (
    <div className="bg-background min-h-full pb-24">
      ...
      {/* Stolen vehicle alerts entry point */}
      <div className="px-4 pb-3">
        <button
          onClick={() => navigate('/stolen-vehicles')}
          className="w-full flex items-center gap-3 p-3 bg-white border border-neutral-200 rounded-xl text-left hover:bg-neutral-50 transition-colors"
        >
          <div className="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center flex-shrink-0">
            <AlertTriangle className="w-5 h-5 text-red-600" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-foreground">Stolen Vehicle Alerts</p>
            <p className="text-xs text-muted-foreground">Report or search community-reported thefts</p>
          </div>
          <span className="text-sm font-semibold text-red-600 flex-shrink-0">View</span>
        </button>
      </div>
