...
import { ArrowLeft, MapPin, Car, Bike, Package } from 'lucide-react';
...
const Marketplace = () => {
  ...
  const filteredListings = activeCategory
    ? mockMarketplaceListings.filter(l => l.category === activeCategory)
      ...
      <div className="px-4 pb-8">
        <div className="grid grid-cols-2 gap-3">
          {filteredListings.map((listing) => (
            <button
              key={listing.id}
              className="bg-white rounded-xl overflow-hidden shadow-sm border border-border text-left hover:shadow-md transition-shadow"
            >
              {/* Image Placeholder */}
