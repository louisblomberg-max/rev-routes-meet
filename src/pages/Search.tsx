...
export default function SearchPage() {
  ...
  return (
    <div className="mobile-container min-h-screen md:max-w-2xl md:mx-auto" style={{ background: '#ECEAE4' }}>
      <div style={{ background: '#FFFFFF', borderBottom: '1px solid #E8E4DC', padding: '12px 16px', paddingTop: 'max(env(safe-area-inset-top), 12px)', display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={() => navigate(-1)} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', padding: 4 }}>
          <ArrowLeft size={22} color="#4A443D" />
        </button>
        <div style={{ flex: 1, position: 'relative' }}>
          <SearchIcon size={16} color="#8C867E" style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)' }} />
          <input
            autoFocus
            value={query}
            onChange={e => { setQuery(e.target.value); handleSearch(e.target.value); }}
            placeholder={`Search ${context === 'all' ? 'everything' : context}...`}
            style={{ width: '100%', background: '#F2EFE9', border: 'none', borderRadius: 12, padding: '11px 14px 11px 38px', fontSize: 15, color: '#111', outline: 'none' }}
          />
        </div>
      </div>

      <div style={{ padding: 16 }}>
        {loading && <p style={{ color: '#8C867E', fontSize: 14 }}>Searching...</p>}
        {!loading && query && results.length === 0 && (
          <p style={{ color: '#8C867E', fontSize: 14, textAlign: 'center', paddingTop: 32 }}>No results for "{query}"</p>
        )}
        {results.map((r, i) => (
          <button
            key={`${r._type}-${r.id}-${i}`}
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', background: '#FFFFFF', border: '1px solid #E8E4DC', borderRadius: 14, padding: 12, marginBottom: 8, cursor: 'pointer', textAlign: 'left' as const }}
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: '#8C867E', marginTop: 2, textTransform: 'capitalize' as const }}>
